...
function M998.pvFixCheck()
	...
	ISEnterVehicle["start"] = function(self)
		...
		local vehicle = self.vehicle
			local vehicle = self.vehicle
			if 	vehicle and (
				string.find( vehicle:getScriptName(), "92amgeneralM998" )) then
...
function M998.pvFixSwitch(player)
...
function M998.pvFixClear(player)

		player:SetVariable("KI5vehicle", "False")
end

Events.OnGameStart.Add(M998.pvFixCheck);
--Events.OnGameStart.Add(KI5.pvFixSwitch);
Events.OnExitVehicle.Add(M998.pvFixClear);
Events.OnSwitchVehicleSeat.Add(M998.pvFixSwitch);